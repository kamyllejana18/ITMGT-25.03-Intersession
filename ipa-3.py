...
def relationship_status(from_member, to_member, social_graph):
    ...
    print(f"from_member: {from_member}")
...
def tic_tac_toe(board):
    ...
    for row in range(rows):
        if all(board[row][col] == board[row][0] for col in range(cols)) and board[row][0] != "":
            return board[row][0]
    ...
    if all(board[i][cols - 1 - i] == board[0][cols - 1] for i in range(rows)) and board[0][cols - 1] != "":
